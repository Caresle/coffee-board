...
import { useDetailArchiveStore } from "../../../_states/boardDetails.state"
...
export default function ArchiveBoardModal() {
	const { show, update, item } = useDetailArchiveStore(state => state)
	const queryClient = useQueryClient()
	const { selectedBoard } = useBoardGlobal()

	const mut = useMutation({
		mutationFn: boardDetailsService.archive,
		onSuccess: () => {
		...
		<Dialog open={show} onOpenChange={value => update({ show: value })}>
			<DialogContent className="max-w-none min-w-[35%]">
				<DialogHeader>
					...
					<DialogDescription>
						This action will archive the board
					</DialogDescription>
				</DialogHeader>

				<p>Are you sure you want to archive this board?</p>
